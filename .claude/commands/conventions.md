...
| Path | Purpose |
|------|--------|
| config/ | Cluster YAMLs (e.g. hpcfund.yaml, frontier.yaml), job.template, config/tools/*.yaml |
...
Templates for sweeps use the same structure but with **list values** for dimensions to sweep (see `/project:sweeps`).
...
Each YAML defines one or more tools (name, conflicts, etc.). The job generator loads all `config/tools/*.yaml` and builds the tool list. Use `omnihub-generate-job --help` to see the current tool list. No hardcoded tool list; tools are discovered at runtime.
...
- **OMNIHUB_RESULTS_DIR** — Same as results_dir; used by omnihub-run and apps.
...
Each Slurm task gets its own overlay (`$results_dir/.overlay.$SLURM_PROCID`). `scripts/omnihub-apptainer-env.sh` is sourced inside the container to set ROCm paths and create a per-task pip cache. The overlay provides a writable `/opt/venv` layer.

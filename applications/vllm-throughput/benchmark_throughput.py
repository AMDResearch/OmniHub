...
class BenchmarkThroughput:
    def run_vllm(
        ...
        if not use_new_beam_search_impl:
            ...
            start = time.perf_counter()
            llm.beam_search(
                prompts, beam_width=n, max_tokens=output_len, ignore_eos=True
            )
            end = time.perf_counter()
        return end - start
    ...
    def run(self):
        ...
        if args.dataset is None:
            ...
            requests = [
                (prompt, args.input_len, args.output_len)
        ...
        if args.backend == "vllm":
            ...
            if args.async_engine:
                ...
                elapsed_time = uvloop.run(self.run_vllm_async(*run_args))
            else:
                elapsed_time = self.run_vllm(*run_args, args.use_new_beam_search_impl)
        elif args.backend == "hf":
            assert args.tensor_parallel_size == 1
            elapsed_time = self.run_hf(
                requests,
                args.model,
                tokenizer,
                args.n,
    ...
    def __init__(self, custom_args: list, config: dict):
        ...
        if config:
            # Convert the YAML configuration to a list of arguments
            config_args = []
            for key, value in config.items():
                config_args.append(f"--{key}={str(value)}")

            # Combine the config arguments with the remaining command-line arguments
            final_args = config_args + custom_args
        else:
...
def run(*args, **kwargs):
    BenchmarkThroughput(*args, **kwargs).run()

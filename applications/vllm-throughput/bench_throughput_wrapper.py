#!/usr/bin/env python3
"""
vLLM throughput Benchmark Wrapper for Omnihub
"""
...
def run_vllm_bench_throughput(extra_args, config):
    resolve_model_path(config, {})
    cli_args = extra_args + [
        f"--{k.replace('_', '-')}={str(v)}" for k, v in config.items()
    ]
    argv = ["vllm", "bench", "throughput"] + cli_args
...
def run(extra_args, config):
    run_vllm_bench_throughput(extra_args, config)
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="vLLM bench throughput wrapper")
    parser.add_argument("--app-config", required=True)
    args, extra = parser.parse_known_args()
    with open(args.app_config) as f:
        config = yaml.safe_load(f)
    config.pop("entrypoint", None)
    run_vllm_bench_throughput(extra, config)
